...
public class ScrollChartView extends View {
    ...
    public ScrollChartView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        //初始化一些参数
        mSelectedColor = ContextCompat.getColor(context, R.color.c_2C9DFF);
        ...
        mTextSize = UiUtils.dp2px(context, 10);//未选中文字大小
        mTextSelectedSize = UiUtils.dp2px(context, 10);//选中文字大小
     ...
     * @param dataList
     */
    public void setData(List<String> times, List<Integer> dataList) {
       Integer  min = Collections.min(dataList);
    ...
    private void initValue() {
        ...
        mGridPaint.setStyle(Paint.Style.STROKE);//绘制虚线 必须为stroke
        PathEffect pathEffect = new DashPathEffect(new float[]{10,5},0);
        mGridPaint.setPathEffect(pathEffect);//绘制虚线
...
//            canvas.drawRect(left, 0, right, bottom, mGridPaint);
            canvas.drawLine(left,0,right,bottom,mGridPaint);
        }
    }
