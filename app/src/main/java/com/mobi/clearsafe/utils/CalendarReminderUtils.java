...
public class CalendarReminderUtils {
     ...
     */
    @RequiresApi(api = Build.VERSION_CODES.N)
    public static void addCalendarEvent(Context context, String title, String description, long reminderTime, long endTime,long previousDate) {
        if (context == null) {
            return;
        }
        int calId = checkAndAddCalendarAccount(context); //获取日历账户的id
        if (calId < 0) { //获取账户id失败直接返回，添加日历事件失败
            Log.e("wwww","获取账户id失败直接返回，添加日历事件失败");
            return;
        }
        ...
        mCalendar.setTimeInMillis(endTime);//设置结束时间
        ...
        values.put(CalendarContract.Reminders.MINUTES,  previousDate/1000/60);// 提前previousDate天有提醒
        values.put(CalendarContract.Reminders.METHOD, CalendarContract.Reminders.METHOD_ALERT);
        Uri uri = context.getContentResolver().insert(Uri.parse(CALENDER_REMINDER_URL), values);
        if (uri == null) { //添加事件提醒失败直接返回
            Log.e("wwww","添加事件提醒失败直接返回");
            return;
        }
    }
