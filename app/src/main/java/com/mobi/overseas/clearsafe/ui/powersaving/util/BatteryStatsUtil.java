package com.mobi.overseas.clearsafe.ui.powersaving.util;

/**
 * @author zhousaito
 * @version 1.0
 * @date 2020/4/15 18:21
 * @Dec 略
 */
...
public class BatteryStatsUtil {
    ...
    private String TAG = "BatteryStatsUtil";
    ...
    private static BatteryStatsUtil sInstance;
    ...
    public static BatteryStatsUtil getInstance() {
        if (sInstance == null) {
            synchronized (BatteryStatsUtil.class) {
                if (sInstance == null) {
                    sInstance = new BatteryStatsUtil();
                }
            }
        }
        return sInstance;
    ...
    public long computeBatteryTimeRemaining() {
        if (!mOnBattery) {
            Log.i(TAG, "mOnBattery" + mOnBattery);
            return -1;
        }
        if (mNumDischargeStepDurations < 1) {
            Log.i(TAG, "mNumDisChargeStepDurations" + mNumChargeStepDurations);
            return -1;
        }
    ...
    public long computeChargeTimeRemaining() {
        ...
        if (mNumChargeStepDurations < 1) {
            Log.i(TAG, "mNumChargeStepDurations" + mNumChargeStepDurations);
            return -1;
        }
    ...
    public void setBatteryState(int status, int plugType, int level, Context context) {
        synchronized (this) {
            ...
            int state = Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP ?
                    windowManager.getDefaultDisplay().getState() : STATE_UNKNOWN;
            ...
            if (onBattery != mOnBattery) {
                ...
                if (onBattery) {
                    if (mLastDischargeStepLevel != level && mMinDischargeStepLevel > level) {
                        ...
                        Log.i(TAG, "mNumDischargeStepDurations（赋值后）-------"
                                + mNumDischargeStepDurations);
                        Log.i(TAG, "mDischargeStepDurations（赋值后）-------"
                    ...
                    if (mLastChargeStepLevel != level && mMaxChargeStepLevel < level) {
                        ...
                        Log.i(TAG, "mNumChargeStepDurations（赋值后）-------"
                                + mNumChargeStepDurations);
                        Log.i(TAG, "mChargeStepDurations（赋值后）-------"
