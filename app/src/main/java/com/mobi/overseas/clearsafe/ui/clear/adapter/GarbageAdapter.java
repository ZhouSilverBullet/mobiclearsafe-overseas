...
public class GarbageAdapter extends BaseMultiItemQuickAdapter<MultiItemEntity, BaseViewHolder> {
    ...
    public GarbageAdapter(List<MultiItemEntity> data) {
        ...
        addItemType(2, R.layout.garbage_ad_item);
...
//                    pbLoad.setVisibility(View.VISIBLE);
            ...
            case 3: {
                ...
                cbMemory.setVisibility(View.GONE);
    ...
    public List<LenFile> changeToRemoveDataIsEmpty() {
        ...
        List<LenFile> pathList = new ArrayList<>();
            ...
            if (datum instanceof GarbageHeaderBean) {
                ...
                for (GarbageBean subItem : subItems) {
                    if (subItem.isCheck) {
                        ...
                        if (subItem.fileList != null && subItem.fileList.size() > 0) {
                            int size = subItem.fileList.size();
                            long currentLen = subItem.fileSize / size;
                            for (File file : subItem.fileList) {
                                //获取file，分别吧长度获取，平分给这些file
                                LenFile lenFile = new LenFile(file.getPath());
                                lenFile.setLen(currentLen);
                                pathList.add(lenFile);
                            }
                        }
                    }
                }
