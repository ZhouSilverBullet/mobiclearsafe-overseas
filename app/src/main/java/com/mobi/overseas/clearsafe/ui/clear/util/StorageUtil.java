...
public class StorageUtil {
    ...
    private static void filterSdCardPath(Set<String> sdCardDirs) {
        ...
        if (!"GT-I9000".equals(Build.MODEL.trim())) {
            return;
        }
     ...
     * @return
     */
    public static int getStoragePercent(Context context, long useSize) {
        SDCardInfo sdCardInfo = getSDCardInfo(context);
        if (sdCardInfo.mFree == 0 && sdCardInfo.mTotal == 0) {
            sdCardInfo.mFree = 1L;
        }
        return (int) (useSize * 100 / (sdCardInfo.mTotal + sdCardInfo.mFree));
    }
